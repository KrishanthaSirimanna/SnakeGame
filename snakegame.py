...
def drawGameTime(gameTime):
    ...
    screen.blit(game_time_numb(105,10))
...
def respawnApples(apples , quantity , sx , sy):
    ...
    while(counter < quantity):
        while(angle > radius):
            ...
            x=SCREEN_WIDTH/2 + radius * math.cos(angle)
            y=SCREEN_HEIGHT/2 + radius * math.sin(angle)
            if((x-APPLE_SIZE == sx or +APPLE_SIZE == sx) and (y-APPLE_SIZE == sy or y+APPLE_SIZE == sy) or radius - angle <= 10):
                continue
...
def main():
    ...
    respawnApple(apples,max_apples , mySnake.x , mySnake.y)
